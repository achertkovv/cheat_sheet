...
```
$ git restore --staged file.txt # перевести файл file.txt из состояния staged обратно в untracked или modified
```
```
$ git restore file.txt # вернуть файл file.txt к последней версии, которая была сохранена через git commit или git add
```
```
$ git reset --hard b576d89 # удалить все незакоммиченные изменения из staging и «рабочей зоны» вплоть до указанного коммита (b576d89)
```
...
```
```
$ git diff a9928ab 11bada1 # вывести разницу между двумя коммитами (a9928ab 11bada1)
```
```
